...
pnpm smoke:pelican      # 鹈鹕骑车 /pelican-bike/ 实机烟测（vendored Three.js 单文件游戏，25 项判据：HTTP 200 / body.ready / canvas 与 window.__pelican / 两处返回入口 / 署名与 .tools 恒为 8 / 零外部请求 / 开局状态机 / 六档视口几何 / Tab 环 / 真实点回主站 / pageerror 与 console.error 双零；实测 25/25，**需先 `pnpm build && pnpm preview --port 4322`**）；PELICAN_BASE_URL 传**站点根**（脚本自己拼 /pelican-bike/，与 FANCY_BASE_URL / UI_SMOKE_BASE_URL 同形，不同于 NICE_BOOKS_BASE_URL / AI_NEWS_BASE_URL 的**完整页面地址**）。默认 http://localhost:4322 而非 127.0.0.1——Astro preview 在 Windows 只绑 [::1]，与 test:fancybox / smoke:ui 同口径。全脚本没有一个 waitForTimeout，等的是 body.ready / body.started / #intro 的 computed visibility 三个真实状态信号
...
- 版本：astro 6.4.8 / @astrojs/react 5.0.7 / @astrojs/svelte 8.1.2 / astro-expressive-code 0.43.1（`^0.43` 而非 0.44）/ @swup/astro 1.8.0（**实际安装 1.8.0，但 `package.json` 的范围仍声明 `^1.7.0`**——本条按 Q45-B 记录该差异而不是顺手改范围：pnpm lockfile 里存的是 specifier，改 `package.json` 不改 lock 会让 CI 的 `pnpm install --frozen-lockfile` 直接失败；要改得连 lock 一起改并实测） / tailwindcss 3.4.x。**Tailwind 不再经 @astrojs/tailwind**——该集成 peer 只到 Astro 5，是当初升级的硬阻碍；现由根 `postcss.config.mjs` 直连 `[tailwindcss/nesting, tailwindcss, autoprefixer]`，顺序照集成原先的实际产出，`autoprefixer` 已转为直接依赖。原配置文件里 import 的 `postcss-import` 既非声明依赖、全站 CSS 又零 `@import`，会让该文件在集成内静默加载失败，已移除；**不要再加回 @astrojs/tailwind**。
- **Tailwind v3 的 `@layer` 是构建期指令，不是 CSS 级联层——产物里根本不存在 `@layer`**（实测：`dist` 内九个 css 文件 `grep -c '@layer'` 全为 0）。因此源码里的 `@layer base/components/utilities` 只影响"被展开到何处"，**冲突一律按「特异性 → 源顺序」判定**，不存在"无层规则胜过 @layer"这种级联层语义。本仓库两处历史结论曾据此误判：① 根字号 `html{font-size:13px}`（写在层内）失效的真因是它被更靠后源顺序的同特异性 `html{font-size:100%}` 压掉，而非层序；② 行内代码 `.prose code{font-size:13px}` 被描述成"关在 `@media (max-width:680px)` 内、桌面永不生效"，实际它在 `@layer components` 内、桌面一直生效，压住它的是另一条规则的更高特异性。推断样式覆盖关系时请直接量计算值，别按层语义推理。
...
- `src/constants/*.json` 与 `src/data/site-stats.json` 的排版由 `scripts/lib/write-json.mjs` 的 `formatJson()` 在生产端保证（`JSON.stringify(x, null, 制表符)` 再过一遍 Prettier），七个产物幂等、`prettier --check ./src` 覆盖它们。旧写法下六个 constants 用同一种序列化，四个只是碰巧没有短数组才合规 —— 短数组（如 `"variants": [480, 720, 1080, 1440]`）会被 Prettier 折行而 `JSON.stringify` 每行一个元素，这正是 `image-manifest.json` / `github-projects.json` 当初被塞进 `.prettierignore` 的原因。**新增写 JSON 产物的脚本请走同一个写入器，别再往 `.prettierignore` 加行**；该文件现在只剩 vendored 的 `src/styles/font-awesome.css` 一条。
- **deploy.yml 的 "Sync Giscus comments" 步骤跑在 `pnpm install` 之前**（`node scripts/sync-site-stats.mjs`，那时 `node_modules` 还不存在），所以这条链路上的脚本只能用 Node 内置模块。`formatJson()` 因此对 prettier 用 `try { await import("prettier") }` 延迟加载、缺失即回落到纯 tab 缩进 —— CI 那次写出的排版只留在 runner 工作区、不回提交，站点内容不受影响；本地装了 devDependencies 才是 Prettier 排版。给这条链路加任何包依赖都会让部署在 13 秒内以 `ERR_MODULE_NOT_FOUND` 失败（实测踩过）。
...
- 文章页的分类/标签链接**只对真的生成出来的路由发**（`src/pages/posts/[...slug].astro`）：合法性判据与 `/tag/[tag]`、`/category/[category]` 的 `getStaticPaths` **同源**，即 `getTagList(publicPosts)`／`getCategoryList(publicPosts)` 的名字集合——这两个函数都先过 `isPublicPost`，所以 `private: true` 文章独占的那一项从不生成页面。无路由的项降为**纯文字 `<span>`**，不是整行删除：私有文章若用的是公开分类（实测 `技术` 就是有路由的），链接照常保留。勿改回无条件发 `<a>`，那会重新制造 404（GitHub issue #41）。冒烟 `pnpm smoke:ui` 的 `#41` 三项判据守着这条，**被检页面清单取自 sitemap** 而不是从站内链接爬——私有文章页不被任何公开页链出，爬是爬不到的。**将来若改 sitemap 收录策略，必须在同一笔 commit 里给那条判据换页面清单来源**，否则它会静默退化成「只查公开页」（spec #42 登记的跨轮硬约束）
...
| 鹈鹕骑车（本站资源） | `/pelican-bike/` 下的 vendored 第三方 Three.js 单文件 3D 小游戏（海岸公路骑行、程序化建模、Verlet 围巾、Gerstner 海浪、合成音效），**不是本站写的代码**。**许可是事实陈述、不是承诺**：上游 `riba2534/claude-opus-5-5-demo` 子目录 `pelican-bike/`、commit `54adc1fb64155f9dd7c1c90ecae7b8185ac540b8`，仓库根与该子目录**都没有 LICENSE 文件**（GitHub API 的 `license` 字段亦为 null），只有其 `package.json` 声明 `"license": "ISC"`；我们原样保留该声明与游戏内的 GitHub/X 署名链接，并**按其声明把标准 ISC 文本补写成 `vendor/pelican-bike/LICENSE`**（版权行 `Copyright (c) 2026 riba2534`，年份取自上游 commit 日期 2026-09-23；那是我方就地落地对方声明的许可，**不是上游随附文件、也不构成我方权利主张**，先例为 `public/vendor/svg3dtagcloud/LICENSE`；本仓库根目录仍无 LICENSE）。**分工**：源码留档 `vendor/pelican-bike/`（11 个 `src/*.js` 与上游逐字节相同，md5 基线在该目录 `README.md`；上游 320 行 / 25,816 字节 / md5 `89528bdab1cc0e3acb3f3716149b632d`，可从 `git cat-file blob 5efd0a3:vendor/pelican-bike/index.template.html` 就地取得，勿依赖 `output/upstream-pelican-bike-54adc1f/`——那是本机 gitignore 的临时对照目录，干净检出上不存在），产物 `public/pelican-bike/index.html` **跟踪提交**（非 gitignore，`git check-ignore -v` 对它无输出）。实测产物 **818,646 字节 / md5 `c043aa0ed9b2086419995a07c8ad15a0` / 纯 LF / 零外部资源**（脚本打印的 `798.3 KB (js 774.0 KB)` 是 UTF-16 字符数，别当字节数用；`dist/` 里那份是 822,109 字节，差额来自构建期的佛祖横幅注入，它遍历 dist 全部 208 个 HTML）。**不参与 `pnpm build`**（也不进 `pnpm check` 与 `prettier --check ./src`，因为它在仓库根 `vendor/` 而非 `src/`），重建纯手工且需显式执行：`cd vendor/pelican-bike` → `npm install --no-package-lock --no-audit --no-fund three@0.186.0 lil-gui@0.21.0 esbuild@0.28.2` → `node build.mjs` → 自行提交产物。三个约束：① **必须 cd 进本目录**——`import.meta.dirname` 只锚**输出**落点，`entryPoints: ['src/main.js']` 与 `readFileSync('index.template.html')` 两个**输入**仍相对 `process.cwd()`（从仓库根跑的实测后果：esbuild 报 `Could not resolve "src/main.js"`、exit 1、产物 md5 与 mtime 均不变，是大声失败而非静默走错），且 `import.meta.dirname` 需 Node ≥ 20.11；② **`vendor/pelican-bike/package.json` 必须在位**——它的 `"type": "commonjs"` 决定 esbuild 的打包语义，删掉它 `node build.mjs` 会 **exit 0 静默产出 815,878 字节（−2,768 B）的不同 bundle**（helper 名 `__esm`/`__commonJS` 只在 `--dev` 未压缩构建里可 grep，压缩后已被 mangle，故**别拿 `grep __commonJS` 打压缩产物当判据**，认 md5）；③ **依赖只落本目录 `node_modules/`**（`.gitignore` 的 `vendor/pelican-bike/node_modules/` 那条已收，实测根 `node_modules/` 内 three/lil-gui/esbuild 三者皆无，隔离成立），`--no-package-lock` 必带（上游 `package-lock.json` 把仅用于其自测的 `playwright-core` 钉到字节内网 registry `bnpm.byted.org`，`npm install` 直接 `EALLOWREMOTE`，因此该 lock 也不 vendor）。相对上游**只改模板三处**（`vendor/pelican-bike/index.template.html`，改后实测 26,117 字节 / **321 行**（上游 320 行，仅第②处新增整行）/ 净 **+301 字节** / md5 `238b31dd5f05edab308e29f0937f4c36`；`<style>…</style>` 整块与 `.tools` 块逐字节未动，**零新增 CSS**）：① 选择器锚点 `<body class="bars">`（约 191 行）加 `data-pagefind-ignore="all"`；② `.intro-links`（约 229 行）在**首位**插一个 `href="/"` 的「返回博客首页」链接；③ HUD 的 `.hud > .brand.panel`（约 238 行）整块由 `<div>` 换成 `<a href="/">`。**游戏页不进博客搜索是两套独立机制叠加（belt-and-braces，勿因"看着冗余"就删属性）**：显式的 `data-pagefind-ignore="all"`（两页 scratch 站的隔离实测，本轮 ledger 记于 2026-09-25：「带属性 → Indexed 0 pages / 0 words；剥掉 → 1 page / 207 words」），以及本站存在 `data-pagefind-body` 页、pagefind 本就忽略无该标记的页（`pnpm build` 日志实读 `Found a data-pagefind-body element on the site. ↳ Ignoring pages without this tag.`，全站 208 个 HTML 只 `Indexed 17 pages`（本次 2026-09-25 `pnpm build` 实读）；ledger 同日记的另一项观察：把属性整站剥掉，页数仍是 17 而不是 18）。保留显式属性的理由是第二条依赖"站内始终有 `data-pagefind-body` 页"这一配置现状，搜索策略一改就静默失效，而显式属性不依赖环境且已单独证明有效。**验证排除的两个坑**：pagefind 正文存在压缩 fragment 里，**明文 grep 中文关键词连必然被索引的正常页都匹配不到**（假阴性），只能读 pagefind 自己的账目（stdout 的 `Indexed N pages` + `fragment/` 文件数）或走真实检索；且 pagefind 对 zh-cn 不分词，所以搜索断言只能写成「结果不含 `/pelican-bike/`」，不能写成「结果为零」。导航入口在 `src/config.ts` 的 `navBarConfig.resourceSite`：`{ name: "鹈鹕骑车", url: "/pelican-bike/", noSwup: true }`，`Navbar.astro`（桌面 hover 下拉）与 `MMenu.astro`（移动端全屏菜单）两处均由该数组数据驱动发 `data-no-swup`，**不需要分别在两个组件里加链接**；`noSwup: true` 与同组 `AI日报`/`每日好书` 是同一约定——凡不进博客 `Layout.astro`、因此没有 swup 容器 `main` 的独立壳页面一律 `noSwup`，交给浏览器整页加载（让 swup 接管只会得到空容器）。**返回入口不做进 `.tools`**：`vendor/pelican-bike/index.template.html` 的 `.tools > *` 恒为 **8**（六个 `icon-btn` + 两个上游署名锚点），实测塞进第 9 个会让右锚定的工具条变宽 36/44px、320px 视口下 `.tools.x` 变成 −28 把最左按钮推出屏外、414px 出现上游没有的重叠，故入口落在 `.brand`，`.tools` 保持逐字节禁改区。**`.brand` 那个 `<a>` 上的 `tabindex="-1"` 必须在**：`vendor/pelican-bike/src/main.js`（约 995 行）键码处理开头是 `if (e.target instanceof HTMLInputElement \|\| e.target instanceof HTMLAnchorElement) return;`——焦点停在锚点上时 W/S/A/D、空格、`U` 全部失效；`.brand` 又是 HUD 里 DOM 顺序最靠前的可聚焦元素，把它变成 `<a>` 等于让锚点当第一个 Tab 停靠点（评审在构建产物上实机复现过键位冻住），而 `src/*.js` 逐字节冻结、又不许新增客户端 JS，故取最小解把它移出 Tab 环（鼠标/触控照旧可点），键盘用户改用第②处那个仍在 Tab 环内的开场卡链接或浏览器后退；**同一隐患上游本就有两处**（`.tools` 里那两个署名锚点被 Tab 到同样冻键位），属原版行为、动它等于改 `.tools`，有意不修，勿当成本仓库的回归上报。回归由 `pnpm smoke:pelican`（`scripts/pelican-bike-smoke.mjs`）守着，2026-09-25 实测 **25/25 全绿**，前置 `pnpm build && pnpm preview --port 4322`，`PELICAN_BASE_URL` 传**站点根**。**`pnpm dev` 下 `/pelican-bike/` 的目录形态靠 `astro.config.mjs` 里名为 `pelican-bike-dev-directory-index` 的 dev-only Vite 中间件重写得到**：该页是 `public/` 静态文件而非 Astro 路由，Vite 的 public 中间件按路径直出、不做目录索引重写，缺了它 dev 会 404（构建产物与 GitHub Pages 本来就能解析，所以这是纯 dev 缺口、不是线上问题）；`configureServer` 不参与生产构建，产物字节不受影响。另注：不带尾斜杠的 `/pelican-bike` 在 dev 下仍是 404，与 `/books` 同性质，属全站惯例，勿当成本页缺陷去修。 |
...
- 头部微言轮播（`#header .text`，2026-09-05 重写）：机制复刻原版 AutoScroll（limh.me `global-pjax.js`）——`theme-script.ts` 的 `initHeaderTicker()` 每 4s 将 ul 上滚一条（0.8s ease），`transitionend` 后把首条 li 移到末尾实现无限轮转（无克隆条，任意条数无缝，条数取 `diary.slice(0, 4)`），hover 暂停、移出恢复，`prefers-reduced-motion` 不启动，dataset 守卫防定时器叠加（原版 pjax 重复加载会叠加，勿学）。**节奏 4s/条 = 停留 3.2s + 滑动 0.8s 对齐迁移前 CSS 关键帧版手感，勿改回原版 300ms**（2026-09-05 用户确认）；步长 `-24px` 与 global.css `#header .text li` 行高耦合，改行高须同步。头部在 Swup 容器外，随 `pagefindReady()` 初始化一次即可，勿加进 after-swap hook。**实现只有这一条**：主容器文本块 `#header .text` + `initHeaderTicker()`。曾并存的 CSS-only 死块 `.header-ticker` + `@keyframes ticker-scroll`（含窄屏副本）从未被任何标记或脚本引用，已于 2026-09-22 删除——不要再把它当「缺 `prefers-reduced-motion` 配对的活跃动画」上报
- **微言轮播链接「无悬停视觉反馈」是既定设计（2026-09-23 裁决，勿重开）**：`#header .text a{color:#fff}`（`src/styles/global.css`，选择器锚点，行号约 1957）以 id 特异性 (1,1,1) 压掉通用 `a:hover{color:var(--colorful-green)}`（约 1770 行，(0,1,1)），且 `#header .text` 全族没有任何 `:hover` 的 `text-decoration` 规则，于是头部那 4 条 `<a href="/diary/">` 悬停时**既不变色也不出线**。站长判定**保持无反馈**：不要补下划线、不要换色、不要给它加 `::after` 线条（补它属于新增视觉行为，须重新过裁决）。**这条 id 规则是承重结构**——删它或降它的特异性等于悄悄给头部开启绿色 hover，动通用 `a:hover` 前必须先想清楚这点。两件事**不在**本次「无 hover」的范围内、也不得顺手删除：hover 暂停滚动（`initHeaderTicker()`，纯 JS 行为）与 `:focus-visible` 描边（键盘可达性）。回归由缝隙 A 承担（`pnpm smoke:ui` 用 `hoverProbe()` 取静息/悬停计算值，断言 `color` 与 `text-decoration-line` 差值为零）——**缝隙 B 的采集排除表显式含 `#header .text` 整族，对它必然零漂移，不能当验过了**；该判据属「锁现状」型，落地即绿，其红证据由变异验证提供。
...
- **层叠关系表（2026-09-22 票 22 立；下列数值全部是真实浏览器读到的**计算值`z-index`**，不是源文本）**：`.skip-link`(1000000) > `#myhkplayer`(900000，`Layout.astro` 内联 `!important`) > `.site-toast`(100100) > `.colorful_loading_frame`(99999) > `.click-word`(9999) > `.post-navigation a`(1000) > `#nav li ul` 与 `.qrcode-pop`(999)。**谁必须位于谁之上是契约，改任一值前先回到这张表核对**；其余约 50 处散值（`global.css` 40 / `books.css` 8 / vendored 与独立壳若干）为既定关系，本轮有意不动。
  - **陷阱一：`z-index` 是 32 位有符号整数。** 写 `10000000000`、`10000000001` 这类超界值不会报错，浏览器把它们**静默钳到 `2147483647`** —— 于是两条「看起来差 1」的规则算成**同一个数**，谁盖谁退化为绘制顺序侥幸。票 22 修的就是这一对（跳过链接与常驻播放器），现已各自落回可表示区间并留 10 万余量。判据必须读计算值：本仓库的冒烟用 `getComputedStyle`，源文本永远看不出这件事。
  - **陷阱二：Fancybox 灯箱是模态 `<dialog>`，走顶层层（top layer）**，`.fancybox__container` 的计算 `z-index` 是 `auto`。**不要拿 z-index 数值去和它比大小**——顶层层结构上压过一切非顶层内容，比数值会得出「特效字 9999 高于灯箱」的反向结论。实测（票 22 命中测试）：灯箱打开时在容器界面内放特效字与提示做 `elementFromPoint`，命中结果都落在灯箱一侧。
...
- 响应式图片变体：`scripts/generate-lqips.mjs` 在 LQIP 之外生成 WebP 变体到 `public/images/_variants/`（480/720/1080/1440 四档，q82，仅对宽度 > 档位×1.2 的图生成），manifest 尺寸表写入 `src/constants/image-manifest.json`（构建产物，**随仓库跟踪**，读取方是 `src/utils/image-variants.ts`；`public/images/_variants/` 下的变体图才是 gitignore）；文章封面/幻灯片/相册/图片墙经 `src/components/control/ResponsiveImage.astro`（`<picture>` WebP + 原图兜底）渲染。变体目录已加入 LQIP 扫描的 IGNORE_DIRS，勿删；中文路径 URL 编码规则与 album-scanner 一致
...
- 标签药丸 `.blogtags`（**样式挂点是类 `.blogtags`**，锚点为 `.blogtags a` 与 `.blogtags a::before/::after` 那一组规则，源自原版 colorful-original.css 的 `#blogtags` 那份（2026-09-24 迁移）。**id 只由正文/云集页那一份发射**，侧栏 WidgetTag 那份带类不带 id——同页两个同名 id 会让锚点与辅助技术引用只指到第一处（issue #47）。行号会随编辑漂移，上一版记录就漂了约 220 行，故一律以选择器定位）：6 色轮换 + `::before` 三角 + `::after` 圆点，`/tag/` 两类标签页、`/category/` 两类分类页、侧栏 WidgetTag **五处共用**同一 DOM 结构，勿另写药丸样式；`.tag-count`（×N 数字，11px 白色）与 `a.is-current`（当前标签/分类品牌绿 `--colorful-green` 底 + 三角同色）为仅有的两处新增规则，**必须保持在 nth-child 轮换规则之后**（同 specificity 靠源顺序覆盖），移动位置会丢失高亮。窄屏（`max-width:680px`）另有一条命中区放宽（垂直内距 3→5px，三角与圆点随盒高走），桌面盒值不动。**同页重复 id 由 `pnpm smoke:ui` 的常驻负扫守着**（清单取自 sitemap 全部公开页 + 404 页，2026-09-24 起）：新部件若要发 id，必须发**唯一** id，直接复用旧 id 会当场翻红，勿为此给判据加放行条目
