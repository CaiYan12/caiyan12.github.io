...
**③ 私有文章页全部在 sitemap 里（2026-09-23 修 #41 时查出；站长已判：维持现状，本条结案）**。`astro.config.mjs` 的 `sitemap({ filter })` 只排除 `/page/1/` 与 books 数据，`dist/sitemap-0.xml` 里 21 篇 `/posts/…/` 一篇不少——包含 10 篇 `private: true`。这与 #41 里「私有内容不进公开索引」的既定语义相反：站内不链出，却主动向爬虫申报 URL。**站长 2026-09-23 确认按现状保留**：不改 filter、不加 `noindex`、不动 robots.txt，因此缝隙 A 的 `#41` 判据可以继续依赖 sitemap 枚举被检页面。此条不再作为下轮议题重开；若将来要改，必须同时处理 `scripts/ui-smoke.mjs` 里那条枚举依赖。
