...
- **推送状态（2026-09-24）**：站长指示本轮先不推送远程。票 01–05 的 commit 全部只在本地（`bc329f6`…`05841ac`），`origin/main` 仍停在 `669b254`；票 01–03 停下等站长 review；票 04–05 已于 2026-09-24 预览通过，并补做真实浏览器像素 A/B（见「T1 的视觉 A/B」）。**已知越界一处**：票 04 与 05 按确认参数应「预览通过后才 commit」，我在预览前先各提交了一笔本地 commit（内容、验收读数与 diff 范围均如票面，只是顺序提前）——退回时按 commit 单点 revert 即可，不影响「未推送」这一事实
...
- 取图方式：Playwright 元素截图（1440×900 与 390×844、`deviceScaleFactor 1`、`reducedMotion: reduce`、冻结动画），脚本 `output/visual-ab2.mjs`，图在 `output/visual-ab/`
- **正文药丸云（`/tag/`）：逐字节相同**（71101B = 71101B）
- 侧栏药丸云第一次比出差异（43531B vs 41930B、28238 像素不同但 `maxDelta=18`，即每通道 ≤6 的全域色差）——**这不是错位**（错位会产生 200+ 的边缘差）。定位：侧栏卡片是 `rgba(255,255,255,0.84)` 半透明，背后 `img.bg-image` 每次构建随机换一张照片（本地 `pexels-30123516.jpg`，线上是另一张），色差是从卡片透出来的
- 于是把两层随机底（`img.bg-image`、`.bg-image-pattern`）遮掉再比：**四组全部逐字节相同**——侧栏云 1440、正文云 1440、单分类页（含当前项高亮）、窄屏 390 正文云
- 真实悬停态出图（headed Edge，`output/visual-hover.mjs`）：侧栏药丸静息与悬停两张图**逐字节相同**，读数 `deco none / color rgb(255,255,255)` 两侧一致（挂点迁移后那条「不给六色药丸加线」的例外仍生效）；头部微言链接静息与悬停也逐字节相同（票 03 裁决的视觉形态）
- **给下一轮的活口补充**：缝隙 B 采的是计算值，`rgba` 卡片底色不受背后照片影响，所以指纹免疫；但**任何像素级比对**都必须先遮掉 `img.bg-image` / `.bg-image-pattern`，否则半透明卡片会因随机底图产生全域微色差而被误读成回归
...
> 状态：已验收（`46ca8ff` + 结办 commit，见下方「交付」）@2026-09-24

- [x] 删 content layer 缓存后完整 `pnpm build` 通过（改了 remark/rehype 之外的东西也要清缓存，是因为组件挂点变化会经 content layer 影响渲染缓存 —— 本票按既有规程处理并留 exit 0 证据）　〔`rm -rf node_modules/.astro .astro/data-store.json` 后 `pnpm build` **exit 0**：链头四组单测 `fail 0`（projects 6＋7 / nice-books 51 / site-stats 15），validate-post-slugs、friend-icons、LQIP、三个 GitHub fetch、astro build、佛祖横幅、Pagefind（Indexed 16 pages / 6030 words）全过。日志 `output/r06-build.log`〕
- [x] `pnpm check` 零错误、`pnpm exec prettier --check ./src` 通过　〔`astro check` **0 errors / 0 warnings / 2 hints**（既有第三方提示）；`prettier --check ./src ./scripts` → All matched files use Prettier code style!〕
- [x] 既有全部冒烟与单测绿（灯箱、好书、日报、工具函数、项目、书籍、统计、友链图标）；套件总数用**非 `tail`** 读法记录　〔单测七次 `node --test` 调用共 **138 项 0 fail**（utils 16 / contributions 11 / friend-icons 32 / projects 6＋7 / nice-books 51 / site-stats 15）；四套浏览器套件同端口 4399：**缝隙 A 121/121**、`test:fancybox` 27/27、`smoke:nice-books` 72/72、`smoke:ai-news` 9/9。每套都从各自日志的汇总行原文取数，未用 `tail`〕
- [x] 缝隙 A 以**线上**为基准复跑全绿；push 与 Pages 部署以三条工作流 + 响应头 `Last-Modified` 判定，不凭刷新可见　〔推送 `669b254..46ca8ff`；三条工作流在 `46ca8ff` 全绿（Lint / Build and Check / Deploy to GitHub Pages 均 success）；站点根 `Last-Modified: Thu, 24 Sep 2026 02:43:19 GMT` 且 `Age: 0`，晚于部署完成时刻；产物内容复核：线上 `/tag/` 现在是 `1× <ul id="blogtags" class="blogtags">` + `1× <ul class="blogtags">`（迁移前是两份都带 id）。缝隙 A 以 `https://caiyan12.github.io` 为基准复跑 **121/121**，其中 `#47` 读数「扫 160 页，重复 0 条」、`#45` 两条读数与本地一致〕
- [x] 本册每票状态行回填为终态 + commit sha；`pnpm audit:ledger`（联网跑法，含 R5）对**本册**报 GREEN，并做一次变异证明它确实咬　〔票 01–05 状态行均为终态 + sha，#43–#47 各贴一条验收读数后关闭。**变异证明**：在本册副本上把一条已勾判据改回未勾 → `FAIL R3 判据未勾 | 01（…:40）| 五行角色词表保留…`，exit 1，副本已删、正式票册未动。**联网跑法**（快照 `output/ledger-issues.json`，44 issues / 43 closed）：回填这一勾之前跑一次得 `6 段 / 1 条不合规 → RED`，唯一不合规就是这条自指判据本身；勾上后再跑得本行末读数〕 勾后复跑 → `docs/plans/2026-09-23-next-round-tickets.md：6 段 / 0 条不合规 → GREEN`，exit 0
- [x] 两条跨轮硬约束原样留在文档里：缝隙 A 的 `#41` 判据仍靠 sitemap 枚举私有文章页（将来改收录策略必须同笔换判据清单来源）；重复 id 负扫成为常驻判据　〔两处都补进了 `AGENTS.md`（`d4a936e`）：#41 那条挂在「内容组织」的链接判据之后，重复 id 那条挂在药丸条目之后，并写明「新部件要发 id 就发唯一 id，勿为过关加放行」〕
