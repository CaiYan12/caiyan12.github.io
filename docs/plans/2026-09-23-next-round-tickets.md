...
- [ ] 缝隙 A 以**线上**为基准复跑全绿；push 与 Pages 部署以三条工作流 + 响应头 `Last-Modified` 判定，不凭刷新可见　〔**未做**：站长指示本轮先不推送远程，`origin/main` 仍是 `669b254`。线上复跑判据必须以推送后的部署为准，现在跑只会量到迁移前的产物〕
- [ ] 本册每票状态行回填为终态 + commit sha；`pnpm audit:ledger`（联网跑法，含 R5）对**本册**报 GREEN，并做一次变异证明它确实咬　〔票 01–05 状态行已是终态 + sha，票 06 自身待推送后结办，所以联网跑法此刻必然 RED（R5：#43–#47 五张镜像未关）。**变异证明已做**：在本册副本上把一条已勾判据改回未勾 → `FAIL R3 判据未勾 | 01（…:40）| 五行角色词表保留…`，exit 1；副本已删，正式票册未动（`git status` 干净）〕
...
- [ ] 构建改写的 `src/constants/github-contributions.json` 与 `github-projects.json` 已 `git checkout --` 还原，工作树干净、`HEAD` == `origin/main`　〔前两项已成立（每次构建后都还原，`git status` 空）；`HEAD` == `origin/main` 待推送〕
- [ ] `.design-flow.json` stage 与 next 更新　〔stage 已置 `verification`；本票结办后再改 `next`〕
