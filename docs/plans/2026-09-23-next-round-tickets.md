...
> 状态：进行中 @2026-09-24（本地可验的四条已达成；余下三条全部卡在「推送需站长同意」这一步）
...
- [ ] 本册每票状态行回填为终态 + commit sha；`pnpm audit:ledger`（联网跑法，含 R5）对**本册**报 GREEN，并做一次变异证明它确实咬　〔票 01–05 状态行已是终态 + sha；#43–#47 五张镜像已各贴一条验收读数并关闭。**变异证明已做**：在本册副本上把一条已勾判据改回未勾 → `FAIL R3 判据未勾 | 01（…:40）| 五行角色词表保留…`，exit 1，副本已删。联网 GREEN 要等本票镜像 #48 关闭后才能成立（R5 把它也算在内），故这一条留到最后一笔回填〕
