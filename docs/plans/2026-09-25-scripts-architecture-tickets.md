...
- [x] 缝隙 A 以线上为基准复跑全绿（推送部署后，`Last-Modified` 判生效）　〔推送 `4416cd1..ed949b0`（fast-forward，`git ls-remote` 事先核过远端未动）；三条工作流在 `ed949b0` **全部 success**（`Build and Check` / `Lint` / `Deploy to GitHub Pages`），站点根响应头 `Last-Modified: Fri, 25 Sep 2026 02:30:13 GMT` 且 `Age: 0` → 以头部判定生效，不以刷新可见为准。线上复跑 `UI_SMOKE_BASE_URL=https://caiyan12.github.io node scripts/ui-smoke.mjs` → **`合计 121 项，失败 0 项`**（`output/t09-prod-ui2.log`），`#47` 在线上扫 160 页重复 0 条；ai-news 线上 `结果：9/9 通过`。**第一次线上跑读成 20 项红，红因是我自己把 base 传成带结尾斜杠的站点根**：`base + "/posts/x/"` 拼出 `//posts/x/`，浏览器按协议相对 URL 解析（host = `posts`），于是 17 段报错归属判据全被 `Invalid URL` / `replaceState` 拒绝灌满。产物无责（norm 与基线逐行相同、CI 全绿），故已把台子的判形改成看 pathname、根路径又带斜杠时额外警告一句，并加一条测试（13 → 14 项），见 `fix(smoke)` 那一笔〕
- [x] 每票状态行回填终态 + sha；`pnpm audit:ledger`（含 R5）对本册报 GREEN　〔票 01–08 八段状态行均为「已验收 + 在 HEAD 链上的 sha」，本段随之转终态。**两次读数都留档**：回填之前联网自检为 RED，不合规**只**出在本段自身的四条自指判据（含这一条）；把本段与状态行补齐后重跑 → **`9 段 / 0 条不合规 → GREEN`**。**中间还有一次假红，成因在自检这一侧**：刷新 issue 快照之前，R5 把九条镜像全报「未关闭」——该规则读的是本地 `output/ledger-issues.json`，那份快照是开票时留的、issue 后来关了它不知道；按脚本头注释的 recipe（`curl -H "Authorization: Bearer $(gh auth token)" …/issues?state=all`，长中文与令牌都不经 argv）重取后 #50–#58 一律 `closed`，R5 随之归零。教训同 `*_BASE_URL` 那条：**先证自检的输入是新的，再信它报的红**。中途另修一处我自己的记录错误：票 07 的 sha 曾按另一会话给的 `ec73a13` 写入，而该对象在本仓库不存在，是 **R4** 报红抓出来的，真值 `ec70aec`（`550eddf` 改）〕
...
- [x] 构建改写的两个 constants 文件已还原，工作树干净、`HEAD` == `origin/main`　〔两次完整构建各自 `git checkout -- src/constants/github-contributions.json src/constants/github-projects.json` 还原（断网那次 fetch 全部回落旧缓存、实际零改动）；终版确定性重建不跑 fetch 链，因此不再脏任何文件。推送为 fast-forward（事先 `git ls-remote` 核过远端仍在 `4416cd1`，未 force、未 rebase 他人提交），推完 `git rev-parse HEAD origin/main` 两个 sha 相同〕
- [x] `.design-flow.json` stage 与 next 更新　〔stage → `verification`、updated → 2026-09-25、history 追加本轮一条（21 条）、decisions 追加三条（产物不变量口径 / 台子 interface 边界 / 形态回显为何按 pathname 判），`next` 写清留给下一轮的四件。该文件被 `.gitignore` 忽略，是本地状态文件，故不进 commit——`node -e JSON.parse` 核过仍合法〕

**证据**：本地部分 = `output/t09-build2.log`（完整链 `FULL_EXIT=0`）、`output/t09-det.log` + `output/t09-final.sha{,.norm}`（终版产物 = 开工前基线）、`output/t09-s{1,2,3,4}-*.log`（四套冒烟 9 / 27 / 72 / 121 全绿、退出码全 0）。线上部分 = `output/t09-prod-ui2.log`（缝隙 A **合计 121 项，失败 0 项**）与 `output/t09-prod-ainews.log`（`结果：9/9 通过`），三条工作流在 `ed949b0` 全 success，站点根 `Last-Modified: Fri, 25 Sep 2026 02:30:13 GMT` + `Age: 0` 判生效。自检两次读数记在第 6 条判据里。

> 中断与裁定记录 @2026-09-25：① **一次外网中断没有换来一次假绿**：第一次完整构建死于 OG 图取字体（AGENTS.md 记过的环境抖动，四路 curl 全 `000`），当时 `dist` 残缺（113 HTML、`dist/og` 0、无 `pagefind`），于是票 09 所有依赖产物的判据**一律留白**、没有提前打勾；等外网恢复重跑到 `FULL_EXIT=0` 才逐项补齐（`output/t09-build.log` 与 `output/t09-build2.log` 两份都在）。② 另一会话反复报来的「console 零报错门禁」（它的票 10 / issue #79 / #81）经七路核验在本工作副本的任何提交与文件里都不存在：`git status` 干净、HEAD 是本人的 `ec70aec`（票 07）、`git show HEAD:scripts/ui-smoke.mjs` 内 `checkClean(` 仍是 **17** 处且 `全页零` / `KNOWN_THIRD_PARTY` 命中 **0**、`HEAD:package.json` 无 `console-error-gate`、四套冒烟在 HEAD 与工作树里的 `checkClean` 数分别为 17 / 0 / 0 / 0、`find /d/pages -maxdepth 3 -name 'console-error*'` 无结果、其引用的四个 sha 在 `git cat-file -t` 下全部 "Not a valid object name"、`gh issue view 79` not found。站长裁定「**既然不存在，那就不做**」，故本批判据数仍为 **9 / 27 / 72 / 121**，未为它增删一条。③ 顺带纠正一处归属：`ec70aec` 是本人撰写提交的票 07，不是对方的提交。
