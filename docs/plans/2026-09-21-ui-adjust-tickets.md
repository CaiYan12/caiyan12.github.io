...
- [x] ~~搬回三态，仅作用于正文真按钮；剧透块保持其藏字所需的暗底样式~~ **作废（不是已实施）**：站长 2026-09-21 裁决走票面降级条款「只记录、不新增规则」，也不引入 `.post-context p > button, .post-context li > button` 的窄化写法，本票不产 commit。此勾只表示该判据已按裁决结办。
...
- [x] 覆盖已核实的落点集合：全站链接 hover、下拉项 hover、侧栏 hover、卡片标题 hover、桌面菜单焦点态、头部文本块装饰伪元素　〔六个落点逐个查过，其中**三处本就合规、未改**：正文链接／文章标签／站脚 hover 已带下划线，桌面菜单焦点态与导航 hover 线自 `4f0d678` 就在（`#menu-index > li > a:hover::after` 与 `#nav li ul a:hover::after`，我先轮的重复声明已回退 `9ec91cf`）；真正缺辅助的是**侧栏链接族与卡片标题**，已补 `text-decoration:underline`（`767ed36`）。**头部文本块装饰伪元素**另判：`#header .text` 全族在 `global.css` 里零 `:hover` 规则，实测该族链接静息 `text-decoration-line:none`＋白字，属「根本没有状态反馈」而非「只靠颜色传达」，不在本票缺陷面内，登记为下轮议题（见证据登记册「整轮收口」第四节）未改码〕
...
- [x] 预览：五处共用该 DOM 的页面同屏比对　〔站长 2026-09-22 预览「通过」；票 23 收口时把这条从人眼补成机检：`scripts/ui-smoke.mjs` 逐处读**标签云集页／单标签页／分类云集页／单分类页／侧栏部件**的 `#blogtags a:not(.is-current)` 计算底色集合，判它恰好等于云页那六色（当前项按设计是品牌绿，故先排除）。单页入口不写死 slug，从云页自己渲染的第一个药丸链接取，标签改名不会假红。缝隙 A 由 111 项增至 **112 项**，实测五处各 6 色、零越界（读数 32/31/6/5/32 格）。**这条补格过程里被评审抓出我自己的两个错，都已改**：① 第一版选择器裸写 `#blogtags`，而云集页上这个 id **有两个元素**（正文云与侧栏部件同用 `TagPillCloud.astro`），两处被并成一处、五处读数其实不是五处——现已按 `#content`／`#sidebar` 限定；② 第一版硬判「每处恰好 6 色」，而全站分类一共 6 个、单分类页排掉当前项只剩 5 格，**它第一次跑就是在这里翻红的**。另把我原先写的「比集合所以能守住 `nth-child` 轮换被改」一句删掉——集合相等对换序不敏感，**这条判据守的是底色单源，轮换顺序仍由缝隙 B 的逐元素指纹守**，两件事不能混着说。变异验证：给 `#sidebar #blogtags a` 注入一个越界底色后该判据翻红（`侧栏部件越界 32 格；32格只出 1 色`），正文侧四处读数不变，还原 dist 后 112/112〕
...
- [x] ~~点击特效浮层不再高于灯箱~~ **票面前提被实测推翻，本条作废**：`.fancybox__container` 挂在模态 `<dialog>` 上走**顶层层**，其计算 `z-index` 是 `auto`，任何数值都盖不住它，所以「特效浮层高于灯箱」这个关系根本不存在（我先前是从 vendored CSS 里 `grep` 到 `z-index:9999` 就断定二者同值冲突——**把 grep 命中当成了规则归属**）。`.click-word` 因此回退为 9999 不动，这条判据由下方那条命中测试替代（已勾：实测特效字与提示的 `elementFromPoint` 命中都落在灯箱一侧）。
...
- **交付后以线上为基准复跑，抓出两处「只会在生产翻红」的判据缺陷（都是我的门禁问题，站点无回归）**：
  1. 灰度反向扫描报 `/archive/ li.myhknow = #888`、`span.index = #888`。这是明月浩空播放器——远端注入、样式也归它自己，本就不在票 08 的收敛范围内。缝隙 B 的排除表早就排掉 `myhk*`，反向扫描漏了这一步，**只有在播放器真加载成功的场合才暴露**，本地永远看不到。已加同样的子树排除。
  2. 下拉子项「零布局位移」偶发量到 2.09px。原因是面板自带 `dropdown-in` 入场动画，我把 idle 取在了动画中途；固定 320ms 等待在生产延迟下不够。已改为「等元素可见 + 等自身几何连续两次一致」再取值。
  修完线上连跑两次 111/111、本地 111/111。**教训**：冒烟一直只在本地跑会把「本地稳」当成「判据对」；凡是依赖远端注入或自带动画的断言，都必须至少对线上跑一次，且不许用固定毫秒数等待。
...
>
> （这一行原先掉在文件末尾、被票册自检判成「票 23 有两条状态行」，2026-09-23 收口时归位）
...
↑ 这一条到本票已经**兑现**：本地全链确实跑了一次（下方判据 1），数据文件按既有规程回收，未留漂移。

- [x] 删 content layer 缓存后完整 `pnpm build` 通过　〔`rm -rf node_modules/.astro .astro/data-store.json` 后 `pnpm build` **exit 0**：链头三组单测各 `fail 0`，validate-post-slugs / LQIP / GitHub 三个 fetch / astro build / 佛祖横幅 / Pagefind 全过。构建按已知行为改写了 `github-contributions.json` 与 `github-projects.json`，已 `git checkout --` 还原（核过 7 个跟踪数据文件的 `git hash-object`，其余四个 constants 与 `site-stats.json` 零漂移）。为让指纹两端读同一份提交态，还原后又跑了一次确定性复检 `pnpm exec astro build` + banner + pagefind（exit 0；首跑在 `/og/*.png` 处因 `fonts.googleapis.com` 连接超时失败，是 AGENTS.md 记录的已知网络抖动，curl 复探 200／0.99s 后原样重跑即过，非代码问题）〕
...
它同时也纠正了我自己一处误判：第一版把「### T3 前置实测」这类散文子块也当成票去要状态行，虚报 1 条——判据过宽不是台账的缺陷，是脚本越界，已按票号收敛。**「回填完成」从此是可跑的，不用靠我的眼睛。**

**遗留（未修、已登记）**：`#header .text a` 静息 `text-decoration-line:none` 且该族零 `:hover` 规则，即头部微言轮播的链接没有任何悬停反馈。它是「无状态反馈」而非票 20 的「只靠颜色传达」，且补它属于新增视觉行为须过预览门禁，故只登记不改码。详见证据登记册「整轮收口」第四节。
