...
- **三处票面前提被实测推翻（都已按事实改做，不是照票面硬修）**：
  ...
  3. 票 20「桌面菜单焦点态缺非颜色辅助」不成立：`#menu-index … :focus-visible::after` 与 `#nav li ul a:focus-visible::after` **早就把线展开了**，缺的只是 hover。故这一票是「把 :hover 加进既有触发列表」，不是新造机制；另实测 `#header .text a` 无任何 hover 规则，票面「头部文本块装饰伪元素」一项属需澄清而非缺陷。
