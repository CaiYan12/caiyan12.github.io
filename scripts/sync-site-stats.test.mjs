// Task 3 同步脚本测试：全部通过注入 fetchImpl 模拟，绝不访问 GitHub / GoatCounter，
...
function commentsPage(totalCount, nodes, hasNextPage = false, endCursor = null) {
	return gqlOk({
		data: {
			node: {
				comments: {
...
test("parseGoatCounterHits：空 hits 为 0，取 hits[0].count，畸形报错", () => {
	assert.equal(parseGoatCounterHits({ hits: [] }), 0);
	assert.equal(parseGoatCounterHits({ hits: [{ count: 7 }] }), 7);
	assert.throws(() => parseGoatCounterHits({}));
	assert.throws(() => parseGoatCounterHits({ hits: [{ count: "x" }] }));
	assert.throws(() => parseGoatCounterHits({ hits: [{ count: -1 }] }));
});
...
test("buildSnapshot 校验 key 与计数", () => {
	const snapshot = buildSnapshot({
		...
		views: new Map([["20260320000000", 12]]),
	...
	assert.throws(() =>
		buildSnapshot({
			slugs: ["20260320000000"],
			discussions: new Map(),
			views: new Map([["20260320000000", -1]]),
			generatedAt: "x",
		}),
	);
});
...
test("Discussions 游标分页 + 顶层/回复汇总 + 显式 0 + 未匹配标题被忽略", async () => {
	...
	assert.deepEqual(snapshot.viewsDelta, {});
			...
			syncSiteStats({
				fetchImpl: async (url, opts) => {
					...
					if (url.includes("api.github.com") && body.variables.id) {
						commentCalls += 1;
						if (commentCalls > 3) throw new Error("fixture safety stop");
...
test("GoatCounter 全量配置：429 重试、空 hits 为 0、精确 include_paths", async () => {
	const gcCalls = [];
	const responses = [
		discussionsPage([{ id: "D1", title: `posts/${slugA}/` }], false, null),
		commentsPage(2, []),
		...
		},
		{
			...
			status: 200,
			headers: new Headers(),
			json: async () => ({ hits: [{ path: `/posts/${slugA}/`, count: 5 }] }),
		},
	];
	const fetchImpl = async (url) => {
		if (url.includes("api.github.com")) return responses.shift();
		gcCalls.push(url);
	...
	const env = {
		GITHUB_TOKEN: "t",
		GOATCOUNTER_SITE: "mysite",
		GOATCOUNTER_API_KEY: "gc-key",
		GOATCOUNTER_START: "2026-09-02T00:00:00Z",
	};
	const snapshot = await syncSiteStats({
		fetchImpl,
		outputPath: output,
	...
	for (const slug of slugs.slice(1)) {
		assert.equal(snapshot.viewsDelta[slug], 0);
	}
	// 精确路径与整点时间参数（URLSearchParams 编码）
	assert.ok(
		gcCalls[0].includes(`include_paths=%2Fposts%2F${slugA}%2F`),
...
test("GoatCounter 部分配置直接报错且不碰网络不改输出", async () => {
	const output = tmpFile();
	...
	await assert.rejects(
		syncSiteStats({
			fetchImpl,
			outputPath: output,
			env: { GITHUB_TOKEN: "t", GOATCOUNTER_SITE: "mysite" },
			gapMs: 0,
		}),
		/GoatCounter partially configured/,
	);
	assert.equal(await fs.readFile(output, "utf-8"), "SENTINEL");
	await fs.rm(output, { force: true });
});

test("GoatCounter 403 中止且输出字节不变", async () => {
	const output = tmpFile();
	await fs.writeFile(output, "SENTINEL", "utf-8");
	const responses = [
		discussionsPage([{ id: "D1", title: `posts/${slugA}/` }], false, null),
		commentsPage(1, []),
		{ ok: false, status: 403, headers: new Headers(), json: async () => ({}) },
	];
	const fetchImpl = async (url) => {
		if (url.includes("api.github.com")) return responses.shift();
		return responses.shift();
