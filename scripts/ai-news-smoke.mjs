...
const baseUrl = process.env.AI_NEWS_BASE_URL ?? 'http://127.0.0.1:4321/ai-news/'
...
try {
  ...
  check('本站资源提供 AI日报 入口', mainSiteLinkCount === 2, `${mainSiteLinkCount} 个（桌面/移动）`)
  ...
  check('页面标题正确', (await page.title()) === "AI 日报 - WindowsIt's Music Club")
  check('顶栏提供返回主站', (await page.locator('header a[href="/"][data-no-swup]').count()) === 1)

  await page.locator(itemSelector).first().click()
  await page.waitForTimeout(500)
  const issueHash = new URL(page.url()).hash
  check('详情页 hash 路由生效', issueHash.startsWith('#/issue/'), issueHash.slice(0, 70))

  await page.getByRole('button', { name: '返回', exact: true }).click()
  await page.waitForTimeout(500)
  check('详情页返回列表', new URL(page.url()).hash === '#/')
  check('页脚提供返回主站', (await page.locator('footer a[href="/"][data-no-swup]').count()) === 1)

  const offline = await browser.newPage({ viewport: { width: 1280, height: 900 } })
  collectErrors(offline, offlineErrors, ['Failed to load resource: net::ERR_FAILED'])
  ...
  check('实时 RSS 失败时回退离线快照', (await offline.locator(itemSelector).count()) > 0)
  check('离线快照状态可见', /离线快照/.test(await offline.locator('header').first().innerText()))
  await offline.close()
} finally {
  await browser.close()
}
...
console.log('\n--- console errors ---')
console.log([...errors, ...offlineErrors].length === 0 ? '(none)' : [...errors, ...offlineErrors].join('\n'))

const failed = results.filter((result) => !result.ok)
console.log(`\n结果：${results.length - failed.length}/${results.length} 通过`)
process.exit(failed.length === 0 && errors.length === 0 && offlineErrors.length === 0 ? 0 : 1)
