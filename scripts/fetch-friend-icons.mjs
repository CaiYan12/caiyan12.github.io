...
const DEFAULT_MANIFEST_PATH = path.join(REPO_ROOT, "src", "constants", "friend-icons.json");
...
export function normalizeFriendUrl(value) {
	if (typeof value !== "string" || !value.trim()) fail("friend URL must be a non-empty HTTP/HTTPS URL");
	let parsed;
	try {
...
function relGroup(rel) {
	...
	if (tokens.includes("shortcut") && tokens.includes("icon")) return "shortcut icon";
...
export function parseIconCandidates(html, finalUrl) {
	...
	const grouped = new Map([["icon", []], ["shortcut icon", []], ["apple-touch-icon", []]]);
	function visit(node) {
		if (node?.tagName === "link") {
			const attrs = new Map((node.attrs ?? []).map(({ name, value }) => [name.toLowerCase(), value]));
			const group = relGroup(attrs.get("rel"));
			const href = attrs.get("href");
			const candidate = group && href ? normalizeCandidateUrl(href, finalUrl) : null;
			if (candidate && !grouped.get(group).includes(candidate)) grouped.get(group).push(candidate);
		}
...
async function fetchWithLimits(url, options) {
	...
	for (let redirects = 0; redirects <= maxRedirects; redirects += 1) {
		...
		if (response.status >= 300 && response.status < 400) {
			...
			if (!current) throw new Error("redirect target must use HTTP/HTTPS");
			continue;
		}
...
function localPathTarget(publicRoot, localPath) {
	if (typeof localPath !== "string" || !localPath.startsWith("/") || localPath.startsWith("//")) return null;
...
function timestamp(now) {
	return typeof now === "function" ? new Date(now()).toISOString() : new Date(now ?? Date.now()).toISOString();
}
...
async function fetchImage(url, options) {
	...
	if (!kind) throw new Error("unsupported or mismatched image MIME/signature");
...
async function processFriend(friend, context) {
	...
	if (localAvatar) {
		...
		if (verified) {
			return { status: "fetched", entry: { friendUrl: normalized, localPath: localAvatar, sourceUrl: localAvatar, contentType: `image/${verified.kind === "jpeg" ? "jpeg" : verified.kind}`, byteSize: verified.bytes.byteLength, lastSuccessfulAt: timestamp(context.now) } };
		}
	}
	const candidates = [];
	...
	for (const candidate of candidates) {
		try {
			...
			return { status: "fetched", entry: makeEntry(normalized, image, context) };
		} catch { /* continue through candidates */ }
	}
	try {
		const page = await fetchWithLimits(pageUrl, context);
		pageUrl = page.url;
		const html = new TextDecoder().decode(page.bytes);
		for (const candidate of [...parseIconCandidates(html, page.url), normalizeCandidateUrl("/favicon.ico", new URL(page.url).origin)]) {
...
export async function fetchFriendIcons(options = {}) {
	...
	for (const friend of context.friends) {
		let normalized;
		try { normalized = normalizeFriendUrl(friend.url); } catch {
		...
		if (result.status === "fetched") {
			...
			if (entry.bytes) {
				const target = path.join(context.publicRoot, entry.localPath.slice(1));
