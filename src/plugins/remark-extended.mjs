...
function extractLabel(directive) {
	const idx = directive.children.findIndex((c) => c.type === "directiveLabel");
	if (idx === -1) return "";
...
export function remarkExtended() {
	return (tree) => {
		...
		visit(tree, "containerDirective", (node, index, parent) => {
			...
			if (!parent || index === undefined || !ADMONITION_TYPES.has(name)) return;
			const htmlOpen = {
				type: "html",
				value: `<div class="admonition admonition-${name}">${extractLabel(node)}`,
			};
			const htmlClose = { type: "html", value: "</div>" };
			parent.children.splice(index, 1, htmlOpen, ...node.children, htmlClose);
		});

		// 2. 叶子指令（GitHub 卡片）
		visit(tree, "leafDirective", (node, index, parent) => {
			if (!parent || index === undefined || String(node.name || "") !== "github") return;
			const repo = escapeHtml((node.attributes && node.attributes.repo) || "");
		...
		visit(tree, "textDirective", (node, index, parent) => {
			if (!parent || index === undefined || String(node.name || "") !== "spoiler") return;
			const htmlOpen = {
				type: "html",
		...
		visit(tree, "code", (node, index, parent) => {
			if (!parent || index === undefined || String(node.lang || "").toLowerCase() !== "mermaid") return;
			parent.children.splice(index, 1, {
				type: "html",
		...
		visit(tree, "blockquote", (node, index, parent) => {
			...
			if (!firstText) return;
			const m = /^\[!(note|tip|important|warning|caution)\]\s*/i.exec(firstText.value);
			if (!m) return;
